...
exports.trackPasswordHistory = async (accountId, currentPassword, newPassword) => {
    try {
        ...
        for (let history of passHistory) {
            const isUsed = await authService.verifyPassword(newPassword, history.contrasenia_hash);
            if (isUsed) {
                return { success: false, message: 'No puedes usar una contraseña que ya hayas utilizado.' };
            }
        }
        ...
        throw new Error('Error al rastrear el historial de contraseñas: ' + error.message);
    }
};
